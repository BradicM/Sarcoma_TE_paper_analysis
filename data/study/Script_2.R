...
ggplot(rbind(features2,features5[-1,]),
       aes(x=names, y=s1, label = round(s1, 1))) + 
  geom_lollipop(point.size = 3, point.colour = "cadetblue") +
  coord_flip() +
  #theme_minimal() +
...
TE_sig<-rownames(TE_and_Cellularity_model_features$glmnet_results_df)[rownames(TE_and_Cellularity_model_features$glmnet_results_df) %nin% c("(Intercept)")]
TE_features<-data.frame(TE_sig) 
colnames(TE_features)<-"TE_features"
