...
#(Figure 1A,B,C), a Kaplan-Meier analysis and plot for Immune hot/cold groups and PFS (months) (Figure 1D),
...
pdf(file = "FIG.1.pdf",   # The directory you want to save the file in
    width = 8.27, # The width of the plot in inches
    height = 11.69) # The height of the plot in inches

#COMBINE HEATMAPS vertically  
FIG1_ABC = ht1AB %v% ht1C
FIG.1_ABC<-draw(FIG1_ABC)

dev.off()

############################################################################################################################

#             KAPLAN MEIEIR ANALYSIS AND PLOTS FOR Immune hot/cold, Figure 1D and Supplemental Fig 2
...
all.equal(colnames(normalized_counts),rownames(mat_for_heatmap_ordered),Ordered_pheno$CMO_ID)

IKZF1_Bcell<-data.frame(normalized_counts[rownames(normalized_counts) %in% c("IKZF1"),],mat_for_heatmap_ordered[,"B cell"])
colnames(IKZF1_Bcell)<-c("IKZF1","B cell")
