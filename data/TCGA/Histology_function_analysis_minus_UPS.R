...
get_TCGA_histology_KM_calcualtions_minus_UPS<- function(cohort){
  ...
  
  {
    ...
    fit_select <- coxph(Surv(OS_months, OS_status)~Immune_subtypes  + `pathologic tumor size` + `short histo`, data=Ordered_pheno_select)
    ...
    fit_IKZF1_select<- coxph(Surv(OS_months, OS_status)~IKZF1  + pathologic.tumor.size + short.histo, data=For_KLM_IKZF1_select)
    ...
    fit_TEscore_select <- coxph(Surv(OS_months, OS_status)~TE_features_levels  + pathologic.tumor.size + short.histo, data=For_KM_TEscore)
