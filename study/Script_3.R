...
######################## Correlation between IKZF1 expression and B-cell proportions; Supplemental figure 6A
all.equal(rownames(mat_for_heatmap_ordered),Ordered_pheno$CMO_ID,colnames(normalized_counts))

mat_for_heatmap_ordered<-data.frame(mat_for_heatmap_ordered)
reg1<-ggplotRegression(lm(mat_for_heatmap_ordered$B.cell~ normalized_counts[rownames(normalized_counts) %in% c("IKZF1"),]+as.numeric(as.factor(Ordered_pheno$`Abbreviation for Figures`))+ as.numeric(as.factor(Ordered_pheno$batch))))
